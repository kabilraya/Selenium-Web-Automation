...
with SB (
    ...
    expired_data_id = 1133
    #here you inspect and put the first expired div's data-post-id
    project_nodes = tree.xpath(f"//div[@data-post-id='{expired_data_id}']/preceding-sibling::div")
    ...
    for node_idx, node in enumerate(project_nodes,start=1):
        bid_information = node.xpath(".//div[@class='e-n-accordion-item-title-text']")[0].text_content().strip()
        splits = bid_information.split(":",1)
        bid_title = splits[1].strip() if len(splits) > 1 else bid_information
        bid_no = splits[0].strip() if len(splits) > 1 else bid_information[:25].strip()
        print(bid_title)
        print(bid_no)
        ...
        for file_idx, file_url in enumerate(file_links, start = 1):
            ...
            url = urljoin("https://www.doc.alabama.gov/",url)
