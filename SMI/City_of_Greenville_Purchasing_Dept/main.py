...
with SB (
    ...
    all_bids = tree.xpath("//div[@class='bidItems listItems']/div[@class='listItemsRow bid']")
    ...
    for node_idx, node in enumerate(all_bids,start=1):
        ...
        for _,directed_link in enumerate(directed_links,start=1):
            ...
            directed_url = urljoin("https://www.greenvillenc.gov/",directed_url)
            sb.uc_open_with_reconnect(directed_url)
            sb.sleep(3)
            page_source = sb.get_page_source()
            sb.sleep(2)
            tree = html.fromstring(page_source)
            file_links = tree.xpath("//span[@class='BidDetail']/p[position()>1]/a")
            ...
            for file_idx, file in enumerate(file_links, start = 1):
                ...
                file_url = urljoin("https://www.greenvillenc.gov/",file_url)
