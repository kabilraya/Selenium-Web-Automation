...
with SB (
    ...
    project_nodes = tree.xpath("//div[@class='faux-table']/div[position()>1]")
    ...
    for node_idx, node in enumerate(project_nodes,start=1):
        ...
        bid_title = node.xpath("./div[1]")[0].text_content().strip()
        
        bid_no = node.xpath("./div[2]")[0].text_content().strip()
        bid_due_date = node.xpath("./div[4]")[0].text_content().strip()
        ...
        directed_link = node.xpath("div[2]/a")[0].get("href","").strip()
        directed_link = urljoin("https://www.ridemcts.com/",directed_link)
        ...
        file_links = tree.xpath("//article[@class='excellence-detail__wrapper']//a")
        if not file_links:
            continue
