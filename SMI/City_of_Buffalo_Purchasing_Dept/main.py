...
with SB (
    ...
    for node_idx, node in enumerate(project_nodes,start=1):
        ...
        bid_no_xpath = f"//div[@class='bidItems listItems']/div[not(contains(@class,'bidsHeader listHeader'))][{node_idx}]/div[1]/span[2]"

        if sb.is_element_visible(bid_no_xpath):
            print("Found the element")
            bid_no_text = sb.get_text(bid_no_xpath).strip()
            bid_no_text = bid_no_text.split(".",1)[-1].strip()
            if bid_no_text == "NA":
                bid_no = bid_title[:25]
            else:
                bid_no = bid_no_text
        ...
        for directed_link in directed_links:
            ...
            directed_url = urljoin("https://www.buffalony.gov/",directed_url)
            ...
            for file_idx, file_link in enumerate(file_links, start = 1):
                ...
                file_url = urljoin("https://www.buffalony.gov/",file_url)
