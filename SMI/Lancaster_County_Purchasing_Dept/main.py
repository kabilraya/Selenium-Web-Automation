...
with SB (
    ...
    bid_nodes = tree.xpath("//div[@id='indent']")
    ...
    for node_idx, node in enumerate(bid_nodes,start=1): 
        ...
        bid_no = bid_title[:25]
        due_date_text = node.xpath("./p[1]")[0].text_content().strip()
        match = re.search(
        r'\b(?:Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sept?(?:ember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)\s+\d{1,2},?\s+\d{4}\b',
        due_date_text
        )
    
        if match:
            bid_due_date = match.group(0)
        print(bid_due_date)
        ...
        if is_duplicate_hash:
            ...
            save_nodes_as_pdf(
                        ...
                        base_url="https://www.lancova.com/",
            )
        ...
        file_links = node.xpath("./h3/following-sibling::p//a")
        if not file_links:
            continue
        ...
        for file_idx, file in enumerate(file_links, start = 1):
            ...
            new_file_index = len(bid_details[node_idx]["files_info"]) + 1
            file_url = urljoin("https://www.lancova.com/",file_url)
