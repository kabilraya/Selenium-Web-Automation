...
with SB (
    ...
    bid_nodes = tree.xpath("//div[@class='list-item-container']")
    print(len(bid_nodes))
    ...
    for node_idx, node in enumerate(bid_nodes,start=1): 
        ...
        bid_no = node.xpath(".//p[contains(@class,'reference-number')]/span/following-sibling::text()")
        if not bid_no:
        ...
        bid_due_date = node.xpath(".//p[contains(@class,'applications-closing')]/text()")[0].strip()
        match = re.search(r'[A-Z][a-z]+ \d{1,2}, \d{4}', bid_due_date)
        if match:
            bid_due_date = match.group()
        ...
        directed_link = urljoin("https://www.claremontca.gov/",directed_link)
        ...
        sb.sleep(3)
        tree = html.fromstring(page_source)
        file_links = tree.xpath("//ul[@class='related-information-list']//a |//div[contains(@class,'body-content')]//a[contains(@class,'document ext-pdf')]")
        print(len(file_links))
        for file_idx, file_link in enumerate(file_links, start = 1):
            file_url = file_link.get("href","").strip()
            file_url = urljoin("https://www.claremontca.gov/",file_url)
