...
with SB (
    ...
    bid_nodes = tree.xpath("//table[@id='bidsTable']/tbody/tr")
    print(len(bid_nodes))
    ...
    for node_idx, node in enumerate(bid_nodes,start=1): 
        bid_title = node.xpath("./td[1]/strong")[0].text_content().strip()
        
        bid_no = bid_title[:25].strip()
        bid_due_date = node.xpath("./td[1]/div")[0].text_content().strip()
        ...
        #Open the link
        directed_link = node.xpath("./td[4]//a")
        directed_link = directed_link[0].get("href","").strip()
        directed_link = urljoin("https://www.bryantar.gov/pages/bids/",directed_link)
        
        sb.execute_script("window.open(arguments[0],'_blank')",directed_link)
        sb.sleep(5)
        ...
        sb.sleep(3)
        tree = html.fromstring(page_source)
        if sb.is_element_visible("//div[contains(normalize-space(), 'Additional') and contains(@class,'OutlineElement')]"):

            file_links = tree.xpath("//div[contains(normalize-space(), 'Additional') and contains(@class,'OutlineElement')]/following-sibling::div[.//a][not(contains(normalize-space(),'Comment'))]//a")
        else:
            file_links = tree.xpath("//div[@class='side-tiles']//a")
        print(len(file_links))
        for file_idx, file_link in enumerate(file_links, start = 1):
            file_url = file_link.get("href","").strip()
            file_url = urljoin("https://www.bryantar.gov/",file_url)
            ...
            file = download_files(sb = sb,
                                  file_url=file_url, #Since it has no URL
