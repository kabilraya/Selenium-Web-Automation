...
def regex_date_filter(raw_due_date: str) -> str | None:
    ...
    numeric_matches = list(re.finditer(r'(\d{1,2}/\d{1,2}/\d{4})', raw_due_date))
    ...
    all_matches = numeric_matches + text_matches
    if not all_matches:
        return None
    last_match = max(all_matches, key=lambda m: m.start())

    raw = last_match.group(0)   # note: group(0), no capturing group needed anymore
    if last_match in numeric_matches:
        try:
        ...
        for fmt in ("%B %d %Y", "%b %d %Y"):
            try:
                date_obj = datetime.strptime(raw_clean, fmt)
                return date_obj.strftime("%m/%d/%Y")
            except ValueError:
                continue
