...
with SB (
    ...
    bid_nodes = tree.xpath("//table[@class='rpfbids']//table/tbody/tr[position()=1]")
    ...
    for node_idx, node in enumerate(bid_nodes,start=1):        
        ...
        bid_no = node.xpath("./td[1]/a[@href]/following-sibling::text()")[0].strip()
        if not bid_no:
        ...
        for _, directed_link in enumerate(directed_links):
            #Get the directed URL
            directed_url = directed_link.get("href","").strip()
            directed_url = urljoin("https://www.northglenn.org/",directed_url)
            ...
            for file_idx, file in enumerate(file_links, start = 1):
                ...
                file_url = urljoin("https://www.northglenn.org/",file_url)
