...
with SB (
    ...
    bid_nodes = tree.xpath("//div[@class='rfp-list']/div[contains(@class,'rfp-item')]")
    ...
    for node_idx, node in enumerate(bid_nodes,start=1): 
        ...
        bid_no = node.xpath("./a/strong")[0].text_content().strip()
        
                
        bid_due_date = node.xpath("./p[@class='deadline'][not(contains(normalize-space(),'MANDATORY'))]")[0].text_content().strip()
        ...
        for file_idx, file in enumerate(file_links, start = 1):
            ...
            file_url = urljoin("https://emmetcounty.org/",file_url)
