...
with SB (
    ...
    for node_idx, node in enumerate(project_nodes,start=1):
        ...
        for _, directed_link in enumerate(directed_links,start=1):
            ...
            directed_url = urljoin("https://its.ny.gov/",directed_url)
            sb.uc_open_with_reconnect(directed_url)
            sb.sleep(3)
            page_source = sb.get_page_source()
            time.sleep(3)
            tree = html.fromstring(page_source)
            file_links = tree.xpath("//div[@class='doc-info']//a")
            if not file_links:
                continue
            for file_idx, file in enumerate(file_links, start = 1):
                ...
                file_url = urljoin("https://its.ny.gov/",file_url)
