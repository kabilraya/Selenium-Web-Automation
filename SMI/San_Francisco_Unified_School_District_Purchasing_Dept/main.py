...
with SB (
    ...
    project_nodes = tree.xpath("//table/tbody/tr[contains(normalize-space(),'09/08/2026')]/preceding-sibling::tr")
    ...
    for node_idx, node in enumerate(project_nodes,start=1):
        ...
        bid_title = node.xpath("./td[1]/p")[0].text_content().strip()
        
        bid_no = bid_title.rsplit("(",1)[-1].strip()
        bid_no = bid_no.rsplit(")",1)[0].strip()
        ...
        file_links = node.xpath("./td[1]/ul//a")
        if not file_links:
            continue
        ...
        for file_idx, file in enumerate(file_links, start = 1):
            ...
            file_url = urljoin("https://www.sfusd.edu/",file_url)
            
            download_name = file_url.split("/")[-2]
            print(download_name)
