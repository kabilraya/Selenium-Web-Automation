...
with SB (
    ...
    bid_nodes = tree.xpath("//table[@class='rpfbids']//table/tbody/tr[.//a]")
    print(len(bid_nodes))
    ...
    for node_idx, node in enumerate(bid_nodes,start=1): 
        ...
        if formatted_date:
            try:
                ...
                try:
                    ...
                    print("Couldn't parse the date into an object")
                    continue
        ...
        directed_link = urljoin("https://www.oaklawn-il.gov/",directed_link)
        sb.uc_open_with_reconnect(directed_link)
        sb.sleep(4)
        page_source = sb.get_page_source()
        sb.sleep(2)
        tree = html.fromstring(page_source)
        file_links = tree.xpath("//article//a[contains(@href,'pdf') or contains(@href,'docx')]")
        if not file_links:
            continue
        ...
        for file_idx, file in enumerate(file_links, start = 1):
            ...
            file_url = urljoin("https://www.oaklawn-il.gov/",file_url)
