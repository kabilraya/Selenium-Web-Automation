...
with SB (
    ...
    bid_nodes = tree.xpath("//table[@id='tablepress-5']/tbody/tr")
    print(len(bid_nodes))
    ...
    for node_idx, node in enumerate(bid_nodes,start=1): 
        bid_title = node.xpath("./td[2]/text()[1]")[0].strip()
        
        match = re.search(r'[A-Z]{2,4}-\d{1,3}-\d{1,4}', bid_title)
        bid_no = match.group(0) if match else bid_title[:25].strip()
        bid_due_date = node.xpath("./td[1]")[0].text_content().strip()
        ...
        for file_idx, file in enumerate(file_links, start = 1):
            ...
            file_url = urljoin("https://ualr.edu/",file_url)
