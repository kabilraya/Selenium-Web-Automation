...
with SB (
    ...
    for node_idx, node in enumerate(project_nodes,start=1):
        ...
        bid_no_text = node.xpath("./div[1]/span[2]/text()")
        
        if len(bid_no_text) > 1:
            bid_no = bid_title[:25].strip()
        else:
            bid_no = bid_no_text[0].strip()
            
        if bid_no in seen_bid_no:
            print(f"{bid_no} is repeated so skipping this")
            continue
        ...
        for directed_link in directed_links:
            ...
            directed_url = urljoin("https://www.redmond.gov/",directed_url)
            ...
            if is_duplicate_hash:
                ...
                save_nodes_as_pdf(
                            ...
                            base_url="https://www.redmond.gov/",
                )
            ...
            for file_idx, file_link in enumerate(file_links, start = 1):
                ...
                file_url = urljoin("https://www.redmond.gov/",file_url)
