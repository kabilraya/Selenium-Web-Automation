...
with SB (
    ...
    for node_idx, node in enumerate(project_nodes,start=1):
        ...
        for _,directed_link in enumerate(directed_links):
            ...
            directed_url = urljoin("https://www.somifl.gov/",directed_url)
            ...
            print("num tables:", len(all_tables))
            for t in all_tables:
                print(repr(t.get("summary")), repr(t.get("class")), repr(t.get("id")))
            file_links = tree.xpath("//div[@id='contentarea']/div[3]/div[2]/table/tbody//tbody/tr[last()]//a")
            ...
            for file_idx, file in enumerate(file_links, start = 1):
                ...
                file_url = urljoin("https://www.somifl.gov/",file_url)
