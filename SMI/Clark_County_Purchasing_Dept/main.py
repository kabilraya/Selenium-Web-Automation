...
with SB (
    ...
    project_nodes = tree.xpath("//div[@class='views-row']")
    ...
    for node_idx, node in enumerate(project_nodes,start=1):
        bid_due_date = node.xpath(".//strong[contains(normalize-space(),'Closing Date')]/following-sibling::text()")[0].strip()
        ...
        if date_obj and date_obj <= datetime.today().date():
            continue
        bid_title = node.xpath(".//strong[@class='field-content']")[0].text_content().strip()
        bid_no = node.xpath("./div[2]//strong/following-sibling::text()")[0].strip()
        ...
        for file_idx, file_link in enumerate(file_links, start = 1):
            ...
            file_url = urljoin("https://clark.wa.gov/",file_url)
