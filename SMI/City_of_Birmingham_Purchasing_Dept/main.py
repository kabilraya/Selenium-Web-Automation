...
with SB (
    ...
    bid_nodes = tree.xpath("//article/div[2]/div[position()>1]")
    ...
    for node_idx, node in enumerate(bid_nodes,start=1):        
        ...
        bid_due_date = node.xpath(".//div[contains(@class,'row')][.//*[contains(normalize-space(.), 'Proposals Due') or contains(normalize-space(.), 'Bids Due') or contains(normalize-space(.), 'Deadline to Apply')]]/div[2]/p[1]")[0].text_content().strip()
        ...
        file_links = node.xpath("./div/div[2]/a")
        if not file_links:
            continue
        ...
        for file_idx, file in enumerate(file_links, start = 1):
            ...
            file_url = urljoin("https://www.birminghamal.gov/",file_url)
