...
with SB (
    ...
    bid_nodes = tree.xpath("//ul[@id='subnav-menu-desktop']/li/ul/li/a")
    ...
    for node_idx, node in enumerate(bid_nodes,start=1): 
        bid_link = node.get("href","").strip()
        bid_link = urljoin("https://dakotacountymn.gov/",bid_link)
        sb.uc_open_with_reconnect(bid_link)
        sb.sleep(2)
        page_source = sb.get_page_source()
        sb.sleep(2)
        tree = html.fromstring(page_source)
        info_div = tree.xpath("//div[@aria-label='Two Column Section']/div[2]/div[2]")
        bid_title = tree.xpath("//h1")[0].text_content().strip()
        bid_due_date = info_div[0].xpath(".//p[./b[contains(normalize-space(),'due')]]")[0].text_content().strip()
        ...
        file_links = info_div[0].xpath(".//p[last()]/a")
        if not file_links:
            continue
        ...
        for file_idx, file in enumerate(file_links, start = 1):
            ...
            file_url = urljoin("https://dakotacountymn.gov/",file_url)
