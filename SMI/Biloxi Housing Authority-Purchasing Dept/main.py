...
with SB (
    ...
    for node_idx, node in enumerate(project_nodes,start=1):
        bid_information = node.text_content().strip().split("-",1)
        bid_title = bid_information[1].strip()
        bid_no = bid_information[0].strip()
        ...
        for file_idx, file in enumerate(file_links, start = 1):
            ...
            file_url = urljoin("https://biloxihousing.org/",file_url)
