...
with SB (
    ...
    bid_nodes = tree.xpath("//div[@class='wpfd-categories ']/a[position()>1]")
    ...
    for node_idx, node in enumerate(bid_nodes,start=1):        
        bid_title = node.xpath("./span")[0].text_content().strip()
        bid_no = bid_title.split(" ",1)[0].strip()
        formatted_date = "Not Specified"

        
        print(f"Bid Title: {bid_title}\nBid No.:{bid_no}\nBid Due Date: {formatted_date}")
        sb.hover_and_click(click_selector=f"//div[@class='wpfd-categories ']/a[.//span[normalize-space(text())='{bid_title}']]", hover_selector = f"//div[@class='wpfd-categories ']/a[.//span[normalize-space(text())='{bid_title}']]")
        sb.sleep(5)
        page_source = sb.get_page_source()
        sb.sleep(3)
        tree = html.fromstring(page_source)

        file_links = tree.xpath("//a[@class='wpfd_downloadlink']")
        print(len(file_links))
        #GO BACK FOR ANOTHER BID
        sb.hover_and_click(hover_selector="//span[contains(normalize-space(),'Back')]",click_selector="//span[contains(normalize-space(),'Back')]")
        sb.sleep(5)
        if not file_links:
            continue
        ...
        for file_idx, file in enumerate(file_links, start = 1):
            ...
            file_url = urljoin("https://awwd.com/",file_url)
