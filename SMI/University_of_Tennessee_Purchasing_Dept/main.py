...
with SB (
    ...
    bid_nodes = tree.xpath("//ul[@aria-label='Negotiation Abstracts']/li[@aria-rowindex]")
    print(len(bid_nodes))
    ...
    for node_idx, node in enumerate(bid_nodes,start=1): 
        bid_title = node.xpath(".//a//span")[0].text_content().strip()
        
        bid_no = bid_title.split(":",1)[0].strip()
        bid_due_date = node.xpath(".//oj-input-date-time[contains(@id,'CloseDate')]//div[@class='oj-text-field-readonly']")[0].text_content().strip()
        ...
        #Open the link
        directed_link = f"//ul[@aria-label='Negotiation Abstracts']/li[@aria-rowindex][{node_idx}]//a"
        sb.hover_and_click(hover_selector=directed_link,click_selector=directed_link)
        sb.sleep(5)
        page_source = sb.get_page_source()
        sb.sleep(3)
        tree = html.fromstring(page_source)
        attachments = tree.xpath("//h2[contains(normalize-space(),'Attachments')]/following::ul[not(contains(@aria-rowcount,'0'))]/li[@role='row']")
        print(len(attachments))
        for file_idx, attachment in enumerate(attachments, start = 1):
            #get the file name
            file_name = attachment.xpath(".//div[@class='oj-sm-padding-2x-end']")[0].text_content().strip()
            #This will act as an download name for hashing
            print(file_name)
            ...
            download_button = f"//ul[not(contains(@aria-rowcount,'0'))]/li[{file_idx}]//span[@class='oj-ux-ico-download']"
            file = download_files(sb = sb,
                                  file_url=file_name, #Since it has no URL
                                  script_directory=script_directory,
                                  download_path=download_path,
                                  file_index=new_file_index,
                                  file_hash=file_hash,
                                  xpath = download_button)
            bid_details[node_idx]["files_info"].update(file)
        sb.hover_and_click(hover_selector="//span[@class='oj-ux-ico-nav-left']", click_selector="//span[@class='oj-ux-ico-nav-left']")
        sb.sleep(5)
    has_downloads = any(
        bid["files_info"]
