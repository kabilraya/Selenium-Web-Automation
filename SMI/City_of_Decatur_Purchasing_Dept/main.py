...
with SB (
    ...
    bid_nodes = tree.xpath("//table[contains(@class,'views-table')]/tbody/tr")
    print(len(bid_nodes))
    ...
    for node_idx, node in enumerate(bid_nodes,start=1): 
        bid_due_date = node.xpath("./td[3]/span")[0].text_content().strip()
        ...
        for file_idx, file_link in enumerate(file_links, start = 1):
            ...
            file_url = urljoin("https://www.decaturga.com/",file_url)
