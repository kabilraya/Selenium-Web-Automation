...
with SB (
    ...
    bid_nodes = tree.xpath("//div[@class='view-content']/div[@class='item-list']/ul/li")
    ...
    for node_idx, node in enumerate(bid_nodes,start=1): 
        ...
        bid_no = bid_title[:25]
        bid_due_date = node.xpath("//div[@class='view-content']/div[@class='item-list']/ul/li/div[contains(normalize-space(),'Release Date')]/following-sibling::div[1]/span[2]")[0].text_content().strip()
        ...
        file_links = node.xpath(".//span[contains(@class,'file')]/a")
        if not file_links:
            continue
        ...
        for file_idx, file in enumerate(file_links, start = 1):
            ...
            file_url = urljoin("https://www.hendersonvillenc.gov/",file_url)
