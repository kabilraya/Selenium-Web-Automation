...
with SB (
    ...
    project_nodes = tree.xpath("//table[@id='rpfbids']/tbody//tr")
    ...
    for node_idx, node in enumerate(project_nodes,start=1):
        bid_title = node.xpath(".//a[position()>1]")[0].text_content().strip()
        
        bid_no = bid_title.split(",",1)[1].strip() if len(bid_title.split(",",1)) == 2 else bid_title[:25]
        ...
        directed_links = node.xpath(".//a[position()>1]")
        ...
        for _, directed_link in enumerate(directed_links,start=1):
            directed_url = directed_link.get("href","")
            directed_url = urljoin("https://www.elmhurst.org/",directed_url)
