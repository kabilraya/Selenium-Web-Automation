...
with SB (
    ...
    bid_nodes = tree.xpath("//tr[.//table]/td/table/tbody/tr[1]")
    print(len(bid_nodes))
    ...
    for node_idx, node in enumerate(bid_nodes,start=1): 
        ...
        bid_title = node.xpath("./td[1]/a[position()>1]")[0].text_content().strip()
        
        match = re.search(r'^([A-Z]{2,5}\s?\d{2,4}-\d{1,4})', bid_title)
        if match:
            bid_no = match.group(1)
            bid_title = bid_title[match.end():].strip(" -")
            bid_title = bid_title.strip()
        ...
        directed_links = node.xpath("./td[1]/a[position()>1]")
        ...
        for directed_link in directed_links:
            ...
            tree = html.fromstring(page_source)
            time.sleep(3)
            file_links = tree.xpath("//div[@id='post']//a[not(contains(@href, 'bidder-signup'))]")
            if not file_links:
                continue
