...
with SB (
    ...
    for node_idx, node in enumerate(bid_nodes,start=1): 
        bid_due_date = node.xpath("./td[8]/span/following-sibling::text()")[0].strip()
        ...
        if date_obj and date_obj <= datetime.today().date():
            continue
        bid_title = node.xpath("./td[7]/span/following-sibling::text()")[0].strip()
        ...
        directed_link = urljoin("https://longbeachbuys.buyspeed.com/",directed_link)
        ...
        file_links = tree.xpath("//tr[contains(normalize-space(),'File Attachments')][./td[contains(@class,'t-head-01')]]//a")
        print(len(file_links))
        for file_idx, file_link in enumerate(file_links, start = 1):
            
            
            file_name = file_link.text_content().strip()
            file_hash = generate_md5_hash(ecgain = ecgains, bidno = bid_no, filename = file_name )
            #Click based file downloads 
            # Creating the xpath of the element
            clickable_link = f"//tr[contains(normalize-space(),'File Attachments')][./td[contains(@class,'t-head-01')]]/td[2]/a[{file_idx}]"
