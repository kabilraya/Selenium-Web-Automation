...
with SB (
    ...
    bid_nodes = tree.xpath("//table[not(contains(@class,'cookielawinfo-winter'))]/tbody/tr[.//a]")
    print(len(bid_nodes))
    ...
    for node_idx, node in enumerate(bid_nodes,start=1): 
        tds_count = len(node.xpath("./td"))
        if tds_count > 3:
            bid_title = node.xpath("./td[3]/a[1]")[0].text_content().strip()
            bid_due_date = node.xpath("./td[4]")[0].text_content().strip()
        else:
            bid_title = node.xpath("./td[2]/p[1]/a")[0].text_content().strip()
        ...
        for file_idx, file_link in enumerate(file_links, start = 1):
            ...
            file_url = urljoin("https://lemoore.com/",file_url)
