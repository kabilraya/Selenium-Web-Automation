...
def regex_date_filter(raw_due_date: str) -> str | None:
    ...
    Extracts a date from text in forms like:
      ...
      - 'September 9, 2026'
      - 'Sep 23 2026' / 'Sept. 23, 2026' / 'Sep 23rd 2026'
    Returns a normalized 'mm/dd/yyyy' string, or None if nothing matched.
    """
    if not raw_due_date:
        return None

    text = raw_due_date.replace('\xa0', ' ')  # normalize non-breaking spaces
    ...
    # 2. Month name (full/abbrev, optional trailing '.'), day (optional ordinal suffix), optional comma, year
    ...
    if text_match:
        ...
        candidate = f"{month_str} {day_str} {year_str}"
        ...
        # handles 4-letter abbreviations like "Sept" that %b won't accept
        short_month_candidate = f"{month_str[:3]} {day_str} {year_str}"
        try:
            return datetime.strptime(short_month_candidate, "%b %d %Y").strftime("%m/%d/%Y")
        except ValueError:
            print(f"Matched text-date pattern but failed to parse: {candidate}")
