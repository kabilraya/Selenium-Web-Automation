...
with SB (
    ...
    all_bids_links = tree.xpath("//div[@id='main-content']/div[4]/div/div[2]/div[2]/div/div[1]/div/div[1]/a")
    ...
    for node_idx, node in enumerate(all_bids_links,start=1):
        #We first get into this bid and collect the information
        url = node.get("href","").strip()
        url = urljoin("https://www.ccprc.com/",url)
        ...
        sb.sleep(5)
        tree = html.fromstring(page_source)
        info_div = tree.xpath("//div[@id='descriptionTab']/div")
        
        
        bid_no = info_div[0].xpath(".//p[./strong][1]")[0].text_content().strip()
        bid_no = bid_no.split(":",1)[-1].strip()
        print(bid_no)
        bid_title = info_div[0].xpath(".//p[./strong][2]")[0].text_content().strip()
        bid_title = bid_title.split(":",1)[-1].strip()
        print(bid_title)
        bid_due_date = info_div[0].xpath(".//p[strong[contains(normalize-space(), 'Closing Date/Time:')]]")[0].text_content().strip()
        ...
        file_links = info_div[0].xpath(".//p[strong[contains(normalize-space(), 'Related Documents:')]]/following-sibling::p/a")
        ...
        for file_idx, file in enumerate(file_links, start = 1):
            ...
            file_url = urljoin("https://www.ccprc.com/",file_url)
