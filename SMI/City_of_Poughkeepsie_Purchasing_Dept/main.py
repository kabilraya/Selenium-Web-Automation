...
from functions import download_files,regex_date_filter,save_nodes_as_pdf, is_direct_download
...
with SB (
    ...
    bid_nodes = tree.xpath("//div[@class='bidItems listItems']/div[not(contains(@class,'bidsHeader listHeader'))]")
    ...
    for node_idx, node in enumerate(bid_nodes,start=1): 
        ...
        directed_link = node.xpath("./div[1]/span[1]/a")
        ...
        directed_url = urljoin("https://www.cityofpoughkeepsie.gov/",directed_url)
        ...
        info_table = "//table[translate(@summary,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz')='bid details'] | //div[@class='fr-view responsiveEditor']/table[@role='presentation']"
        ...
        if is_duplicate_hash:
            ...
            save_nodes_as_pdf(
                    ...
                    base_url="https://www.cityofpoughkeepsie.gov/",
                    )
        ...
        file_links = tree.xpath("//table[@role='presentation']//table//tr[contains(normalize-space(),'Related Documents')]/following-sibling::tr//a")
        if not file_links:
            continue
        ...
        for file_idx, file in enumerate(file_links, start = 1):
            ...
            file_url = urljoin("https://www.cityofpoughkeepsie.gov/",file_url)
