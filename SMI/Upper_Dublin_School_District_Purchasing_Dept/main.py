...
with SB (
    ...
    sb.sleep(3)
    sb.switch_to_frame("//div[@class='embed-wrapper']/iframe")
    sb.sleep(2)
    ...
    bid_nodes = tree.xpath("//div[@class='flip-entries']")
    print(len(bid_nodes))
    ...
    for node_idx, node in enumerate(bid_nodes,start=1): 
        bid_title = node.xpath("./div[1]//a")[0].text_content().strip()
        ...
        file_links = tree.xpath("//div[@class='flip-entries']//a")
        if not file_links:
            continue
        ...
        for file_idx, file in enumerate(file_links, start = 1):
            ...
            file_url = urljoin("https://www.udsd.org/",file_url)
