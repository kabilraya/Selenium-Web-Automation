...
with SB (
    ...
    project_nodes = tree.xpath("//div[@role='listitem' and contains(normalize-space(),'September 22')]/preceding-sibling::div")
    ...
    for node_idx, node in enumerate(project_nodes,start=1):
        bid_due_date = node.xpath(".//p[contains(@class,'applications-closing')]")[0].text_content().strip()
        

        bid_title = node.xpath(".//h2")[0].text_content().strip()
        bid_no = node.xpath(".//p[contains(@class,'reference-number')]/span/following-sibling::text()")[0].strip()
        ...
        directed_link = node.xpath(".//a")[0].get("href","").strip()
        if not directed_link:
            continue
        directed_link = urljoin("https://www.cityoftracy.org/",directed_link)
        ...
        info_table = "//div[@class='development-application-container']/div/div[1]"
        ...
        if is_duplicate_hash:
            ...
            save_nodes_as_pdf(
                        ...
                        base_url="https://www.cityoftracy.org/",
            )
        ...
        file_links = tree.xpath("//div[@class='development-application-container']/div/div[1]//p[contains(normalize-space(),'ADDENDA')]/following-sibling::p[./a]/a")
        ...
        for file_idx, file_link in enumerate(file_links, start = 1):
            ...
            file_url = urljoin("https://www.cityoftracy.org/",file_url)
