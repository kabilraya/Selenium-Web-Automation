...
with SB (
    ...
    for node_idx, node in enumerate(project_nodes,start=1):
        ...
        for directed_link in directed_links:
            ...
            directed_url = urljoin("https://www.schuylercountyny.gov/",directed_url)
            ...
            sb.sleep(2)
            tree = html.fromstring(page_source)
            bid_title = tree.xpath("//table[@summary='Bid details']/tbody/tr[1]//span[@class='BidDetailSpec']")[0].text_content().strip()
            bid_no = bid_title[:25].strip()
            bid_due_date = tree.xpath("//table[@role='presentation' and contains(@style,'background-color')]//tr[contains(normalize-space(),'Closing Date/Time:')]/following-sibling::tr[1]")[0].text_content().strip()
            ...
            if is_duplicate_hash:
                ...
                save_nodes_as_pdf(
                            ...
                            base_url="https://www.schuylercountyny.gov/",
                )
            ...
            for file_idx, file_link in enumerate(file_links, start = 1):
                ...
                file_url = urljoin("https://www.schuylercountyny.gov/",file_url)
