...
with SB (
    ...
    for node_idx, node in enumerate(project_nodes,start=1):
        ...
        bid_title = node.xpath("./td/strong[1]")[0].text_content().strip()
        if not len(bid_title.split("-",1)) == 2:
            bid_title = bid_title + node.xpath("./td/strong[2]")[0].text_content().strip()

        
        bid_no = bid_title.split("-",1)[-1].strip()
        bid_due_date = node.xpath("./td/strong[contains(normalize-space(),'Opening Date')]/following-sibling::text()[1]")[0].strip()
