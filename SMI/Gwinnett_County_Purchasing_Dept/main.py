...
with SB (
    ...
    project_nodes = tree.xpath("//ul[contains(@class,'tabular-list-group')]/li[position()>1]")
    ...
    for node_idx, node in enumerate(project_nodes,start=1):
        bid_due_date = node.xpath(".//p[contains(normalize-space(),'Opening Date')]")[0].text_content().strip()
        ...
        if date_obj and date_obj <= datetime.today().date():
            continue
        bid_title = node.xpath("./div[1]/p[2]")[0].text_content().strip()
        bid_no = node.xpath(".//strong/a")[0].text_content().strip()

        
        print(f"Bid Title: {bid_title}\nBid Number: {bid_no}\nBid Due Date: {formatted_date}")
        file_links = node.xpath(".//a[not(contains(@href,'mailto')) and not(contains(normalize-space(),'Virtual'))]")
        if not file_links:
            continue
        ...
        for file_idx, file_link in enumerate(file_links, start = 1):
            ...
            file_url = urljoin("https://www.gwinnettcounty.com/",file_url)
