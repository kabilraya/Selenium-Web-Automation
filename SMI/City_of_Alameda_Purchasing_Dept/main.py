...
with SB (
    ...
    project_nodes = tree.xpath("//table//tr[contains(normalize-space(),'Tuesday, August 25, 2026, by 2:00PM')]/preceding-sibling::tr")
    ...
    for node_idx, node in enumerate(project_nodes,start=1):
        ...
        formatted_date = regex_date_filter(raw_due_date=bid_due_date)
        ...
        for directed_link in directed_links: 
            ...
            if not directed_url:
                continue
            directed_url = urljoin("https://www.alamedaca.gov/",directed_url)
            sb.uc_open_with_reconnect(directed_url)
            sb.sleep(3)
            page_source = sb.get_page_source()
            sb.sleep(5)
            tree = html.fromstring(page_source)
            file_links = tree.xpath("//div[@id='multiAccordionBody1']//a")
            ...
            for file_idx, file_link in enumerate(file_links, start = 1):
                ...
                file_url = urljoin("https://www.alamedaca.gov/",file_url)
