...
with SB (
    ...
    project_nodes = tree.xpath("//button[@class='accordion']")
    BID_INFO_PATTERN = re.compile(
    r'^(?P<title>.+?)\s*(?:[-–]\s*)?(?P<bid_no>Section\s*#[\w-]+)\s*[-–]\s*(?P<due>.+)$',
    ...
    for node_idx, node in enumerate(project_nodes,start=1):
        ...
        info = node.xpath("./text()")[0].strip()
        match = BID_INFO_PATTERN.match(info)
        if not match:
            print(f"Could not parse bid info: {info}")
            continue

        bid_title = match.group("title").strip()
        bid_no = match.group("bid_no").strip()
        bid_due_date = match.group("due").strip()
        ...
        file_links = node.xpath("./following-sibling::div[1]//a")
        ...
        for file_idx, file in enumerate(file_links, start = 1):
            ...
            file_url = urljoin("https://www.champaigncountyil.gov/highway/",file_url)
