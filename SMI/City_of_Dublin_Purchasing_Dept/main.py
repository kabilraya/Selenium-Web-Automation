...
with SB (
    ...
    bid_nodes = tree.xpath("//h4[contains(normalize-space(),'Request for Proposals')]/following-sibling::ul[.//a]/li")
    print(len(bid_nodes))
    ...
    for node_idx, node in enumerate(bid_nodes,start=1): 
        bid_title = node.xpath("./h6[.//a]//a")[0].text_content().strip()
        
        bid_no = bid_title.rsplit("-",1)[0].strip()
        bid_due_date = node.xpath("./h6[.//a]//span/text()")[0].strip()
        match = re.search(r'([A-Z][a-z]+)\s(\d{1,2}),\s(\d{4})', bid_due_date)
        ...
        for file_idx, file_link in enumerate(file_links, start = 1):
            ...
            file_url = urljoin("https://www.cityofdublin.org/",file_url)
