...
with SB (
    ...
    bid_nodes = tree.xpath("//ul[contains(normalize-space(),'PROJECTS OUT TO BID')][.//a]/li/ul/li")
    print(len(bid_nodes))
    ...
    for node_idx, node in enumerate(bid_nodes,start=1): 
        bid_title = node.xpath("./text()[1]")[0].strip()
        
        number_pattern = r'\d+(?:[.\-]\d+)+'
        bid_no = None
        m = re.match(rf'^({number_pattern})\s*-\s*', bid_title)
        if m:
            bid_no = m.group(1)
        else:
            m = re.search(rf'({number_pattern})\s*$', bid_title)
            if m:
                bid_no = m.group(1)
        if bid_no is None:
            bid_no = bid_title[:25]
        ...
        for file_idx, file_link in enumerate(file_links, start = 1):
            ...
            file_url = urljoin("https://www.ycusd.org/",file_url)
