...
def regex_date_filter(raw_due_date: str, default_year: int | None = None) -> str | None:
    """
    Extracts a date from text in forms like:
      - '9/09/2026' or '09/9/2026'
      - 'September 9, 2026'
      - 'September 15th'          (no year — infers one)
      - 'September 15th by 3:00 PM'
    Returns normalized 'mm/dd/yyyy', or None if nothing matched.
    """
    if not raw_due_date:
        return None

    # Strip ordinal suffixes: 15th -> 15, 1st -> 1, 2nd -> 2, 3rd -> 3
    cleaned = re.sub(r'(\d{1,2})(st|nd|rd|th)\b', r'\1', raw_due_date, flags=re.IGNORECASE)

    # 1) numeric mm/dd/yyyy
    numeric_match = re.search(r'(\d{1,2}/\d{1,2}/\d{4})', cleaned)
    if numeric_match:
        try:
            return datetime.strptime(numeric_match.group(1), "%m/%d/%Y").strftime("%m/%d/%Y")
        except ValueError as e:
            print(f"Matched numeric pattern but failed to parse: {e}")

    # 2) Month dd, yyyy  (year present)
    text_match = re.search(r'([A-Za-z]+\s+\d{1,2},?\s+\d{4})', cleaned)
    if text_match:
    ...
    # 3) Month dd  (NO year) -> infer one
    text_match_no_year = re.search(r'([A-Za-z]+)\s+(\d{1,2})\b', cleaned)
    if text_match_no_year:
        month_str, day_str = text_match_no_year.groups()
        year = default_year or date.today().year
        for fmt in ("%B %d %Y", "%b %d %Y"):
            try:
                candidate = datetime.strptime(f"{month_str} {day_str} {year}", fmt).date()
                # If that date already passed this year, it's more likely next year's bid
