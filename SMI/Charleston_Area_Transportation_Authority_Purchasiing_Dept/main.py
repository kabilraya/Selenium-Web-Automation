...
with SB (
    ...
    project_nodes = tree.xpath("//h3[contains(normalize-space(),'Open Solicitations')]/following-sibling::div[./h4]")
    ...
    for node_idx, node in enumerate(project_nodes,start=1):
        ...
        bid_title = node.xpath("./h4[1]")[0].text_content().strip()
        bid_no = bid_title[:25].strip()
        bid_due_date = node.xpath("./h4[contains(normalize-space(),'Bids Due:')]/following-sibling::p[1]")[0].text_content().strip()
        ...
        for file_idx, file in enumerate(file_links, start = 1):
            ...
            file_url = urljoin("https://ridecarta.com/",file_url)
