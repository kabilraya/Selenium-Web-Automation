...
def download_files(sb, file_url, script_directory,download_path,file_index, file_hash,form_id = None):
    file = {}
    ...
    if form_id:
        # POST-based download: force the form to open its response in a new tab,
        # then submit it in place, exactly as the site's onclick handler does.
        sb.execute_script(f"""
