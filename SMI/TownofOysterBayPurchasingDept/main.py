...
with SB (
    ...
    for node_idx, node in enumerate(project_nodes,start=1):
        count_td = len(node.xpath("./td"))
        
        bid_title = node.xpath("./td[1]")[0].text_content().strip()
        
        bid_no = node.xpath("./td[1]")[0].text_content().strip()[:25]
        bid_due_date = node.xpath("./td[3]")[0].text_content().strip() if count_td == 3 else node.xpath("./td[4]")[0].text_content().strip()
        
        #extract the date text starting at a particular "Month" from the whole sentence:
        # Due Back and Opens Wednesday August 19, 2026 at 11:00 am
        ...
        bid_due_date = match.group(0) if match else None
        ...
        for file_idx, file in enumerate(file_links, start = 1):
            ...
            file_url = urljoin("https://oysterbaytown.com/",file_url)
