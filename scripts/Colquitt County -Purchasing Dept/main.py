...
with SB (
    ...
    main_node = tree.xpath(
    "//div[@class='post clearfix']/p/node()["
    "count(. | //div[@class='post clearfix']/p/text()[contains(., 'Prison-Security Camera System Upgrade')][1]/preceding-sibling::node())"
    "="
    "count(//div[@class='post clearfix']/p/text()[contains(., 'Prison-Security Camera System Upgrade')][1]/preceding-sibling::node())"
    "]"
    )
    ...
    #Separating the bids using the <a> tag's text saying "(click here for the document in .pdf format)"
    ...
    for node in main_node:
        ...
        if not isinstance(node, str) and node.tag == "br":
            ...
            # Two consecutive <br> = end of current bid
            if br_count == 2:
                if current_bid:
        ...
        current_bid.append(node)

# Add the last bid if there is one
    if current_bid:
        bids.append(current_bid)
    
    for node_idx, bid_node in enumerate(bids,start=1):
        bid_title = bid_node[0].strip()
        bid_no = bid_title[:25]
        full_date = bid_node[2].strip()
        raw_due_date = full_date.split(",",1)[1].strip()
        formatted_date = regex_date_filter(raw_due_date)
        ...
        if formatted_date:
            try:
                ...
                try:
                    ...
                    print(f"Cannot parse the date with error: {e}")
                    continue
        if date_obj and date_obj < datetime.today().date():
            print("The bid has past its due date... skipping this bid")
            continue

        file_links = [link for link in bid_node if not isinstance(link,str) and link.tag=="a"]
        ...
        for file_idx, file_link in enumerate(file_links, start = 1):
            ...
            file_url = urljoin("https://www.colquittcountyga.gov/",file_url)
