...
with SB (
    ...
    project_nodes = tree.xpath("//div[@class='views-row'][.//a[normalize-space()='Agricultural Resiliency Against Tariffs Program']]/preceding-sibling::div[@class='views-row']")
    ...
    for node_idx, node in enumerate(project_nodes,start=1):
        bid_title = node.xpath(".//div[@class='webny-teaser-title']")[0].text_content().strip()
        if len(bid_title.split("-",1)) == 2:

            bid_no = bid_title.split("-",1)[0].strip()
        else:
            bid_no = bid_title[:25]
        due_date_text = node.xpath(".//div[@class='description']")[0].text_content().strip()
        due_date_check = due_date_text.split("due",1)

        if len(due_date_check) == 2:
            bid_due_date = due_date_check[1].strip()            
        else:
            #save the current URL
            main_url = sb.get_current_url()
            #get the directing link and go into that page
            a_tag = node.xpath(".//div[@class='webny-teaser-title']//a")
            date_url = a_tag[0].get("href","").strip()
            date_url = urljoin("https://agriculture.ny.gov/",date_url)
            sb.uc_open_with_reconnect(date_url)
            sb.sleep(3)
            new_page_source = sb.get_page_source()
            new_tree = html.fromstring(new_page_source)
            bid_due_date = new_tree.xpath("//text()[contains(., 'Application Period')]/following-sibling::strong[1]")
            bid_due_date = bid_due_date[0].text_content().split("–", 1)[1].strip()
            sb.uc_open_with_reconnect(main_url)
            sb.sleep(3)
        ...
        directed_links = node.xpath(".//div[@class='webny-teaser-title']//a")
        ...
        for idx, directed_link in enumerate(directed_links):
            directed_url = directed_link.get("href","").strip()
            directed_url = urljoin("https://agriculture.ny.gov/",directed_url)

            sb.uc_open_with_reconnect(directed_url)
            sb.sleep(3)
            page_source = sb.get_page_source()
            tree = html.fromstring(page_source)
            #get all the file links(actual file links)
            
            if sb.is_element_visible("//h3[@class='doc-title']"):
                print("In the page having domain agriculture.ny.gov")
                file_links = tree.xpath("//h3[@class='doc-title']/a")
                
            else:
                print("Currently in a different domain...Proceeding to collect the file links")
                file_links = tree.xpath("//div[@class='wp-block-file']//a[1]") 
            ...
            for file_idx, file_url in enumerate(file_links, start = 1):
                ...
                url = urljoin("https://agriculture.ny.gov/",url)
