...
with SB (
    ...
    project_nodes = tree.xpath("//div[@class='row bids-row']/div[@class='item_container']")
    ...
    for node_idx, node in enumerate(project_nodes,start=1):
        ...
        date_string =  node.xpath(".//div[@class='item_date']")[0].text_content().strip()
        date_time = date_string.split(":",1)[1].strip()
        raw_due_date = date_time.split(",",1)[1].strip()
        ...
        if not directed_links:
            continue
        # If any one link is found we iterate over each link and open it because the files are inside the given link
        for directed_link in directed_links: 
            directed_url = directed_link.get("href", "").strip()
            if not directed_url:
                continue
            directed_url = urljoin("https://www.cityofpeculiar.com/",directed_url)
            sb.uc_open_with_reconnect(directed_url)
            sb.sleep(3)
            page_source = sb.get_page_source()
            sb.sleep(5)
            tree = html.fromstring(page_source)
            main_container = tree.xpath("//div[@class= 'container-fluid mainRow']/div[2]/div[2]/div")[0]
            #Now get all the links on this page which are the file_links
            #Also the bid_no for a bid is here in this page
            bid_no  = main_container.xpath("./div[4]/div[2]")[0].text_content().strip()
            
            #gets all the file links in this site
            file_links = main_container.xpath("./div[@class='related_document_list']//a")
            ...
            for file_idx, file_link in enumerate(file_links, start = 1):
                ...
                file_url = urljoin("https://www.cityofpeculiar.com/",file_url)
